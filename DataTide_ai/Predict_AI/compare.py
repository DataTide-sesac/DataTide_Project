...
# ----- 2번쨰
df2 = item_retail.merge(sea_weather, on="month_date", how="left") 
df2 = df2.merge(ground_weather, on="month_date", how="left") 
df2 = df2.merge(item, on="item_pk", how="left") 
# 날짜 정렬 
df2["month_date"] = pd.to_datetime(df2["month_date"]) 
df2 = df2.sort_values(["month_date"]).reset_index(drop=True)
df2["month_num"] = df2["month_date"].dt.year * 12 + df2["month_date"].dt.month
# -----
