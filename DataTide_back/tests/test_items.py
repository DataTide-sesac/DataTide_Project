...
def test_create_and_read_item(client: TestClient):
    ...
    # Create the item
    response_create = client.post("/api/items/", json=item_payload)
    ...
    response_read_one = client.get(f"/api/items/{item_pk}")
    ...
    response_read_all = client.get("/api/items/")
...
def test_read_nonexistent_item(client: TestClient):
    ...
    response = client.get("/api/items/9999") # Assuming 9999 does not exist
